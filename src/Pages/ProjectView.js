...
function ProjectView() {
    ...
    //console.log((data.posts.data).length);
     ...
     return(
      <motion.div className="projectView"
      initial={{opacity:0}}
      animate={{opacity:1}}
      exit={{opacity:0}}
      ...
      >
           <div className="projects">
                 {categories.map((category,id)  =>(
                       ...
                       <div className = "project" key={id}><Link to={"/Projects/" + category.attributes.UID}>{category.attributes.project_name}</Link></div>


            ))}
             </div>
            <div className="p-container">
            <div className="p-heading"> {data.posts.data[0].attributes.catagory.data.attributes.project_name}</div>
            <div className="text">{data.posts.data[0].attributes.catagory.data.attributes.Projecttext}</div>
                ...
                <div className="p-post" key={id}>
                            ...
                            {post.attributes.media.data.map((image,id2) => {
                                return(
                                   ...
                                   <Link to={"/Projects/view/" +data.posts.data[0].attributes.catagory.data.id + "/" + (id) }>
                                    <img
                                      src={api + image.attributes.url}
                                      alt={image.attributes.formats.small.url}/> </Link>
